-- ==============================================================================
-- PROD: qrc → wo (Work Orders) rename — apply via SQL.
-- ==============================================================================
-- Renames the QR-Checker domain objects qrc_* -> wo_* in place (tables, columns,
-- indexes, constraints, file_assets.owner_type). Same content as the CORRECTED
-- migration 0026 + a checksum fix (prod previously baselined the buggy 0026).
--
-- ⚠️ ATOMICITY: postgres-js/Dokploy reject manual BEGIN/COMMIT. For an atomic
--    apply use the psql CLI:
--      psql "<prod-url>" --single-transaction -v ON_ERROR_STOP=1 -f scripts/db/ops/prod-qrc-to-wo-rename.sql
-- ⚠️ DEPLOY ORDER: run this TOGETHER WITH the wo code deploy — the app queries
--    wo_* tables, so the rename and the code must land together.
-- Requires PostgreSQL 15+. Safe: pure renames, data preserved.
...
-- Record the corrected 0026 checksum (prod baselined the pre-fix version).
UPDATE schema_migrations
   SET checksum = '0c40fdbdf144e27cc6614cf4ab878bbe04658ee1c518e9c98ee87c584c338d63', applied_at = now(), applied_by = 'prod-sql'
 WHERE filename = '0026_rename_qrc_to_wo.sql';
