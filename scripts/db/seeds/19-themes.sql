...
--
-- NOTE: Cada INSERT é condicional — só executa se o customer existir.
-- Isso garante idempotência quando 90-keep-only-dimension.sql foi executado
-- anteriormente (que remove todos os customers exceto Dimension).
-- =============================================================================
...
BEGIN
    ...
    --    Condicional: só insere se o holding customer existir
    -- =========================================================================
    IF EXISTS (SELECT 1 FROM customers WHERE id = v_holding_id) THEN
        ...
        RAISE NOTICE 'Inserted theme: MYIO Platform (holding)';
    ELSE
        RAISE NOTICE 'Skipped theme: MYIO Platform — holding customer % not found', v_holding_id;
    END IF;

    -- =========================================================================
    -- 2. Dimension — tema engenharia (laranja estrutural + cinza escuro)
    --    Sempre inserido (Dimension é o customer principal do seed)
    -- =========================================================================
    IF EXISTS (SELECT 1 FROM customers WHERE id = v_dimension_id) THEN
        INSERT INTO look_and_feels (
            id, tenant_id, customer_id, name, description,
            is_default, mode,
            colors, dark_mode_colors,
        ...
        RAISE NOTICE 'Inserted theme: Dimension Engenharia';
    ELSE
        RAISE NOTICE 'Skipped theme: Dimension — customer % not found', v_dimension_id;
    END IF;

    -- =========================================================================
    -- 3. ACME Industrial — manufatura (aço, azul industrial)
    -- =========================================================================
    IF EXISTS (SELECT 1 FROM customers WHERE id = v_company2_id) THEN
        INSERT INTO look_and_feels (
            id, tenant_id, customer_id, name, description,
            is_default, mode,
            colors,
        ...
        RAISE NOTICE 'Inserted theme: ACME Industrial';
    ELSE
        RAISE NOTICE 'Skipped theme: ACME Industrial — customer % not found', v_company2_id;
    END IF;

    -- =========================================================================
    -- 4. ACME Tech SP (branch) — herda do tema ACME Tech, sobrescreve logo/tagline
    -- =========================================================================
    IF EXISTS (SELECT 1 FROM customers WHERE id = v_branch1_id) THEN
        INSERT INTO look_and_feels (
            id, tenant_id, customer_id, name, description,
            is_default, mode,
            colors,
            ...
            true,
            -- parent_theme_id: only reference if the parent theme exists
            CASE WHEN EXISTS (SELECT 1 FROM look_and_feels WHERE id = v_acme_tech_id)
                 THEN v_acme_tech_id
                 ELSE NULL
            END,
        ...
        RAISE NOTICE 'Inserted theme: ACME Tech SP';
    ELSE
