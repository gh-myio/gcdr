...
# Usage:
#   ./detect-relocations.sh
#   GCDR_API_URL=http://localhost:3015 ./detect-relocations.sh
# =============================================================================
...
OUT_FILE="$SCRIPT_DIR/relocation-plan-$(date +%Y%m%d-%H%M%S).json"
...
if [[ ${#PLANS[@]} -eq 0 || ! -f "${PLANS[0]}" ]]; then
  fail "No action-plan-*.json found in $SCRIPT_DIR"
  exit 1
fi

echo -e "\n${BOLD}=== detect-relocations: ${#PLANS[@]} plan(s) ===${NC}"
