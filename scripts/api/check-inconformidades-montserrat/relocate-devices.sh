...
PLAN_FILE=$(ls -t "$SCRIPT_DIR"/relocation-plan-*.json 2>/dev/null | head -1 || true)
if [[ -z "$PLAN_FILE" || ! -f "$PLAN_FILE" ]]; then
  echo -e "${RED}[FAIL]${NC} No relocation-plan-*.json found. Run detect-relocations.sh first."
  exit 1
fi
...
echo -e "\n${BOLD}=== relocate-devices: $count item(s) | dry-run=$DRY_RUN ===${NC}"
