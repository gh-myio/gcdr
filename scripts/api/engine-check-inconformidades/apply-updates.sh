...
#   ./apply-updates.sh --customer mestre-alvaro --dry-run
#   ./apply-updates.sh --customer montserrat --auth jwt
#   ./apply-updates.sh --customer montserrat --action update_identifier   (only UPDATE_IDENTIFIER)
#   ./apply-updates.sh --customer montserrat --action update              (only UPDATE)
...
LOG_FILE="$CUSTOMER_DIR/apply-updates-$(date +%Y%m%d-%H%M%S).log"
...
echo -e "\n${BOLD}=== apply-updates [$CUSTOMER_NAME]: ${#PLANS[@]} plan(s) | filter=$ACTION_FILTER | dry-run=$DRY_RUN ===${NC}\n"
