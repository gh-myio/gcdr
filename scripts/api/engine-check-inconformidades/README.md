# engine-check-inconformidades

Toolset for investigating and resolving device registration non-conformities between ThingsBoard and GCDR.

All scripts are customer-aware: pass `--customer <name>` to select which customer directory under `customers/` to operate on.

---

## Customers

| Directory | Customer | GCDR Customer ID |
|---|---|---|
| `customers/montserrat/` | Montserrat | `a4c64215-f7eb-4102-80b5-e10b98e2f94e` |
| `customers/mestre-alvaro/` | Mestre Álvaro | `e04046d4-baa4-44e9-a378-4dfebe4140f1` |

Each customer directory contains:
- `config.env` — API key, customer UUID, default asset UUID
- `device-map-*.txt` — input files (7 files per customer)
- Generated output files (reports, action plans, relocation plans, logs)
...
| `relocate-devices.sh` | Reads the latest `relocation-plan-*.json` → calls `POST /devices/:id/move` to move devices to the correct customer |
...
```bash
# Run full pipeline for Montserrat
./run-all.sh --customer montserrat

# Run full pipeline for Mestre Álvaro
./run-all.sh --customer mestre-alvaro

# Checks only (skip action-plan and relocation detection)
./run-all.sh --customer montserrat --checks-only

# With JWT auth
./run-all.sh --customer montserrat --auth jwt
...
```
 ┌─────────────────────────────────────────────────────────────────────┐
 │              INPUT — customers/<name>/device-map files              │
 │                                                                     │
 ...
 └────────────┬──────────────┘
              ...
              │
 ┌────────────▼──────────────────────────────────────┐
 │         customers/<name>/action-plan-*.json        │
 │                                                    │
 ...
 └──────┬──────────────────────┬─────────────────────┘
        ...
        │                      │
        │    ┌─────────────────▼──────────────────────────┐
        │    │    customers/<name>/relocation-plan-*.json  │
        │    │                                             │
        │    │  RELOCATE → device exists in wrong customer │
        │    └─────────────────┬──────────────────────────┘
        │                      │
        │          ┌───────────▼────────────┐
        │          │  relocate-devices.sh   │  POST /devices/:id/move
        │          │  --customer <name>     │
        │          │  (dry-run available)   │
        │          └────────────────────────┘
        │
 ...
 │creates.sh        │         │   --customer <name>    │
 │--customer <name> │         │  PATCH /api/v1/devices │
...
```bash
# Single file
./check-inconformidades.sh --customer montserrat --file device-map-energy-2026-03-10-energy-commonarea.txt

# With JWT auth
./check-inconformidades.sh --customer mestre-alvaro --file device-map-water-2026-03-10-water-stores.txt --auth jwt

# Against local dev server
GCDR_API_URL=http://localhost:3015 ./check-inconformidades.sh --customer montserrat --file device-map-energy-2026-03-10-energy-entry.txt
```

**Output:** `customers/<name>/inconformidades-report-<input-basename>-<timestamp>.json`
...
All flags (except `--customer` and `--checks-only`) are forwarded to each `check-inconformidades.sh` invocation.

**Pipeline steps executed:**

| Step | Script | Skipped with `--checks-only` |
|---|---|---|
| 1 | `check-inconformidades.sh --customer <name>` × N files | — |
| 2 | `generate-action-plan.sh --customer <name>` | ✅ |
| 3 | `detect-relocations.sh --customer <name>` | ✅ |
...
# From a different reports directory
./generate-action-plan.sh --customer montserrat --reports-dir /path/to/reports
```

**Output:** `customers/<name>/action-plan-<basename>.json` per report, structured as:
...
Reads the latest `relocation-plan-*.json` and calls `POST /api/v1/devices/:id/move` for each `RELOCATE` device. Both `newAssetId` and `newCustomerId` are required.
...
./relocate-devices.sh --customer montserrat --dry-run
...
./consolidate-creates.sh --customer montserrat --dry-run
...
./consolidate-creates.sh --customer montserrat --out creates-2026-03-10.txt
```

**Output:** `customers/<name>/consolidated-creates-<timestamp>.txt`
...
```bash
# Dry-run: shows what would be patched, no actual requests
./apply-updates.sh --customer montserrat --dry-run

# Apply all (UPDATE + UPDATE_IDENTIFIER)
./apply-updates.sh --customer montserrat

# Only UPDATE_IDENTIFIER (identifier normalization)
./apply-updates.sh --customer mestre-alvaro --action update_identifier

# Only UPDATE (real field divergences)
./apply-updates.sh --customer montserrat --action update

# With JWT
./apply-updates.sh --customer montserrat --auth jwt

# Against local dev server
GCDR_API_URL=http://localhost:3015 ./apply-updates.sh --customer montserrat
```

**Output:** `customers/<name>/apply-updates-<timestamp>.log` — pipe-delimited log per device:
...
All output files land in `customers/<name>/`:
