...
--
-- Additive-only AND IDEMPOTENT: safe to re-run against a database that already
-- has part (or all) of this schema — enums use DO/EXCEPTION guards, columns use
-- ADD COLUMN IF NOT EXISTS, tables/indexes use IF NOT EXISTS, and seed rows use
-- ON CONFLICT DO NOTHING (so control FLAGS/MASTER already tuned by ops are never
-- reset). Introduces the schema the orchestrator-devices worker needs:
...
ALTER TABLE devices ADD COLUMN IF NOT EXISTS health_status health_status NOT NULL DEFAULT 'UNKNOWN';
ALTER TABLE devices ADD COLUMN IF NOT EXISTS unknown_reason device_unknown_reason;
...
ALTER TABLE devices ADD COLUMN IF NOT EXISTS last_timestamp_telemetry timestamptz;
ALTER TABLE devices ADD COLUMN IF NOT EXISTS last_value_telemetry numeric;
-- per-slave energy pull cursor (advances even on empty windows; never rewinds).
ALTER TABLE devices ADD COLUMN IF NOT EXISTS last_fetch_energy_telemetry timestamptz;
-- freshness policy name (nullable → env default); FK to the book below.
ALTER TABLE devices ADD COLUMN IF NOT EXISTS freshness_policy varchar(50);
...
ALTER TABLE centrals ADD COLUMN IF NOT EXISTS monitoring_enabled boolean NOT NULL DEFAULT false;
-- per-central probe cadence override (nullable → env CENTRAL_CHECK_INTERVAL_SECONDS).
ALTER TABLE centrals ADD COLUMN IF NOT EXISTS check_interval_seconds integer;
-- retry-policy name override (nullable → env CENTRAL_DEFAULT_RETRY_POLICY).
ALTER TABLE centrals ADD COLUMN IF NOT EXISTS retry_policy varchar(50);
...
ALTER TABLE centrals ADD COLUMN IF NOT EXISTS last_gateway_check_at timestamptz;
ALTER TABLE centrals ADD COLUMN IF NOT EXISTS last_gateway_check_latency_ms integer;
ALTER TABLE centrals ADD COLUMN IF NOT EXISTS probe_result varchar(40); -- OK|TIMEOUT|AUTH_ERROR|CONFIG_ERROR|HTTP_5XX|PARSE_FAIL

CREATE INDEX IF NOT EXISTS centrals_monitoring_due_idx
  ON centrals (monitoring_enabled, last_gateway_check_at);

-- ── Retry policy book (RFC-0062 §4) ──────────────────────────────────────────
CREATE TABLE IF NOT EXISTS orchestrator_retry_policies (
...
CREATE TABLE IF NOT EXISTS orchestrator_freshness_policies (
  name                 varchar(50) PRIMARY KEY,
...
-- columns or paging anyone until ops deliberately promotes it. ON CONFLICT DO
-- NOTHING so a re-run never resets flags that ops has already changed.
CREATE TABLE IF NOT EXISTS orchestrator_devices_control (
...
CREATE TABLE IF NOT EXISTS orchestrator_devices_runs (
...
CREATE INDEX IF NOT EXISTS orchestrator_devices_runs_monitor_idx ON orchestrator_devices_runs (monitor, started_at DESC);
...
CREATE TABLE IF NOT EXISTS orchestrator_devices_checks (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS orchestrator_devices_checks_entity_idx ON orchestrator_devices_checks (entity_type, entity_id, created_at DESC);
CREATE INDEX IF NOT EXISTS orchestrator_devices_checks_created_idx ON orchestrator_devices_checks (created_at);
