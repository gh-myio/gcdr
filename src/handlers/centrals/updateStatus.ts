...
import { UpdateCentralStatusSchema } from '../../dto/request/CentralDTO';
...
export const handler: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    const data = UpdateCentralStatusSchema.parse(body);

    const central = await centralService.updateStatus(ctx.tenantId, centralId, data.status, ctx.userId);
