...
import { QuickThemeUpdateSchema } from '../../dto/request/LookAndFeelDTO';
...
export const handler: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    const data = QuickThemeUpdateSchema.parse(body);

    const theme = await themeService.quickUpdate(ctx.tenantId, themeId, data, ctx.userId);
