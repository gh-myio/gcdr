...
import { IMaintenanceTaskRepository } from '../interfaces/wo/IMaintenanceTaskRepository';
import { AppError } from '../../shared/errors/AppError';

const { woMaintenanceTasks } = schema;
...
export class MaintenanceTaskRepository implements IMaintenanceTaskRepository {
  async create(
    ...
    const [row] = await db.insert(woMaintenanceTasks).values({
      tenantId,
      installationId,
  ...
  async getById(tenantId: string, id: string): Promise<MaintenanceTask | null> {
    const [row] = await db
      .select()
      .from(woMaintenanceTasks)
      .where(and(eq(woMaintenanceTasks.tenantId, tenantId), eq(woMaintenanceTasks.id, id)))
      .limit(1);
  ...
  async listByInstallation(tenantId: string, installationId: string): Promise<MaintenanceTask[]> {
    const rows = await db
      ...
      .from(woMaintenanceTasks)
      .where(and(
        eq(woMaintenanceTasks.tenantId, tenantId),
        eq(woMaintenanceTasks.installationId, installationId),
      ))
      .orderBy(woMaintenanceTasks.createdAt);
  ...
  async update(
    ...
    const [row] = await db.update(woMaintenanceTasks)
      .set(updates)
      .where(and(eq(woMaintenanceTasks.tenantId, tenantId), eq(woMaintenanceTasks.id, id)))
      .returning();
  ...
  async delete(tenantId: string, id: string): Promise<void> {
    await db.delete(woMaintenanceTasks)
      .where(and(eq(woMaintenanceTasks.tenantId, tenantId), eq(woMaintenanceTasks.id, id)));
  }

  private mapToEntity(row: typeof woMaintenanceTasks.$inferSelect): MaintenanceTask {
    return {
      id:              row.id,
