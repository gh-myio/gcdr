...
import { IInstallationImageRepository } from '../interfaces/wo/IInstallationImageRepository';
import { AppError } from '../../shared/errors/AppError';

const { woInstallationImages } = schema;
...
export class InstallationImageRepository implements IInstallationImageRepository {
  async create(
    ...
    const [row] = await db.insert(woInstallationImages).values({
      tenantId,
      installationId,
  ...
  async getById(tenantId: string, id: string): Promise<InstallationImage | null> {
    const [row] = await db
      .select()
      .from(woInstallationImages)
      .where(and(eq(woInstallationImages.tenantId, tenantId), eq(woInstallationImages.id, id)))
      .limit(1);
  ...
  async listByInstallation(tenantId: string, installationId: string): Promise<InstallationImage[]> {
    const rows = await db
      ...
      .from(woInstallationImages)
      .where(and(
        eq(woInstallationImages.tenantId, tenantId),
        eq(woInstallationImages.installationId, installationId),
      ))
      .orderBy(woInstallationImages.imageOrder);
  ...
  async update(
    ...
    const [row] = await db.update(woInstallationImages)
      .set(updates)
      .where(and(eq(woInstallationImages.tenantId, tenantId), eq(woInstallationImages.id, id)))
      .returning();
  ...
  async delete(tenantId: string, id: string): Promise<void> {
    await db.delete(woInstallationImages)
      .where(and(eq(woInstallationImages.tenantId, tenantId), eq(woInstallationImages.id, id)));
  }

  async nextImageOrder(tenantId: string, installationId: string): Promise<number> {
    const [r] = await db
      .select({ m: sql<number | null>`max(${woInstallationImages.imageOrder})` })
      .from(woInstallationImages)
      .where(and(
        eq(woInstallationImages.tenantId, tenantId),
        eq(woInstallationImages.installationId, installationId),
  ...
  private mapToEntity(row: typeof woInstallationImages.$inferSelect): InstallationImage {
    return {
      id:             row.id,
