...
} from '../interfaces/wo/IVisitaProductRepository';
import { AppError } from '../../shared/errors/AppError';

const { woVisitaProducts, woVisitaProductImages } = schema;
...
export class VisitaProductRepository implements IVisitaProductRepository {
  async create(
    ...
    const [row] = await db.insert(woVisitaProducts).values({
      tenantId,
      ambienteId,
  ...
  async getById(tenantId: string, id: string): Promise<VisitaProduct | null> {
    const [row] = await db
      .select()
      .from(woVisitaProducts)
      .where(and(eq(woVisitaProducts.tenantId, tenantId), eq(woVisitaProducts.id, id)))
      .limit(1);
  ...
  async listByAmbiente(tenantId: string, ambienteId: string): Promise<VisitaProduct[]> {
    const rows = await db
      ...
      .from(woVisitaProducts)
      .where(and(
        eq(woVisitaProducts.tenantId, tenantId),
        eq(woVisitaProducts.ambienteId, ambienteId),
      ))
      .orderBy(woVisitaProducts.createdAt);
  ...
  async update(
    ...
    const [row] = await db.update(woVisitaProducts)
      .set(updates)
      .where(and(eq(woVisitaProducts.tenantId, tenantId), eq(woVisitaProducts.id, id)))
      .returning();
  ...
  async delete(tenantId: string, id: string): Promise<void> {
    await db.delete(woVisitaProducts)
      .where(and(eq(woVisitaProducts.tenantId, tenantId), eq(woVisitaProducts.id, id)));
  }

  private mapToEntity(row: typeof woVisitaProducts.$inferSelect): VisitaProduct {
    return {
      id:           row.id,
...
export class VisitaProductImageRepository implements IVisitaProductImageRepository {
  async create(
    ...
    const [row] = await db.insert(woVisitaProductImages).values({
      tenantId,
      productId,
  ...
  async listByProduct(tenantId: string, productId: string): Promise<VisitaProductImage[]> {
    const rows = await db
      .select()
      .from(woVisitaProductImages)
      .where(and(
        eq(woVisitaProductImages.tenantId, tenantId),
        eq(woVisitaProductImages.productId, productId),
      ))
      .orderBy(woVisitaProductImages.imageOrder);
    return rows.map((r) => this.mapToEntity(r));
  }
  ...
  async nextImageOrder(tenantId: string, productId: string): Promise<number> {
    const [r] = await db
      .select({ m: sql<number | null>`max(${woVisitaProductImages.imageOrder})` })
      .from(woVisitaProductImages)
      .where(and(
        eq(woVisitaProductImages.tenantId, tenantId),
        eq(woVisitaProductImages.productId, productId),
  ...
  async delete(tenantId: string, id: string): Promise<void> {
    await db.delete(woVisitaProductImages)
      .where(and(eq(woVisitaProductImages.tenantId, tenantId), eq(woVisitaProductImages.id, id)));
  }

  private mapToEntity(row: typeof woVisitaProductImages.$inferSelect): VisitaProductImage {
    return {
      id:           row.id,
      tenantId:     row.tenantId,
