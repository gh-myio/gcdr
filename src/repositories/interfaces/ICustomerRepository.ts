...
export interface ICustomerRepository extends IRepository<Customer, CreateCustomerDTO, UpdateCustomerDTO> {
  ...
  getByIngestionCustomerId(tenantId: string, ingestionCustomerId: string): Promise<Customer | null>;
  ...
  // wo_customer_settings) that the given user has any active
  // role assignment scoped to via `customer:<uuid>`.
  listWoEnabledForUser(tenantId: string, userId: string): Promise<Customer[]>;
}
