...
} from '../interfaces/qrc/IVisitaAmbienteRepository';
import { AppError } from '../../shared/errors/AppError';

const { qrcVisitaAmbientes, qrcVisitaAmbienteImages } = schema;
...
export class VisitaAmbienteRepository implements IVisitaAmbienteRepository {
  async create(
    ...
    const [row] = await db.insert(qrcVisitaAmbientes).values({
      tenantId,
      visitaId,
      name:            data.name,
  ...
  async getById(tenantId: string, id: string): Promise<VisitaAmbiente | null> {
    const [row] = await db
      .select()
      .from(qrcVisitaAmbientes)
      .where(and(eq(qrcVisitaAmbientes.tenantId, tenantId), eq(qrcVisitaAmbientes.id, id)))
      .limit(1);
  ...
  async listByVisita(tenantId: string, visitaId: string): Promise<VisitaAmbiente[]> {
    const rows = await db
      ...
      .from(qrcVisitaAmbientes)
      .where(and(
        eq(qrcVisitaAmbientes.tenantId, tenantId),
        eq(qrcVisitaAmbientes.visitaId, visitaId),
      ))
      .orderBy(qrcVisitaAmbientes.createdAt);
  ...
  async update(
    ...
    const [row] = await db.update(qrcVisitaAmbientes)
      .set(updates)
      .where(and(eq(qrcVisitaAmbientes.tenantId, tenantId), eq(qrcVisitaAmbientes.id, id)))
      .returning();
  ...
  async delete(tenantId: string, id: string): Promise<void> {
    await db.delete(qrcVisitaAmbientes)
      .where(and(eq(qrcVisitaAmbientes.tenantId, tenantId), eq(qrcVisitaAmbientes.id, id)));
  }

  private mapToEntity(row: typeof qrcVisitaAmbientes.$inferSelect): VisitaAmbiente {
    return {
      id:               row.id,
...
export class VisitaAmbienteImageRepository implements IVisitaAmbienteImageRepository {
  async create(
    ...
    const [row] = await db.insert(qrcVisitaAmbienteImages).values({
      tenantId,
      ambienteId,
  ...
  async listByAmbiente(tenantId: string, ambienteId: string): Promise<VisitaAmbienteImage[]> {
    const rows = await db
      .select()
      .from(qrcVisitaAmbienteImages)
      .where(and(
        eq(qrcVisitaAmbienteImages.tenantId, tenantId),
        eq(qrcVisitaAmbienteImages.ambienteId, ambienteId),
      ))
      .orderBy(qrcVisitaAmbienteImages.imageOrder);
    return rows.map((r) => this.mapToEntity(r));
  }
  ...
  async nextImageOrder(tenantId: string, ambienteId: string): Promise<number> {
    const [r] = await db
      .select({ m: sql<number | null>`max(${qrcVisitaAmbienteImages.imageOrder})` })
      .from(qrcVisitaAmbienteImages)
      .where(and(
        eq(qrcVisitaAmbienteImages.tenantId, tenantId),
        eq(qrcVisitaAmbienteImages.ambienteId, ambienteId),
  ...
  async update(
    ...
    const [row] = await db.update(qrcVisitaAmbienteImages)
      .set(updates)
      .where(and(eq(qrcVisitaAmbienteImages.tenantId, tenantId), eq(qrcVisitaAmbienteImages.id, id)))
      .returning();
  ...
  async delete(tenantId: string, id: string): Promise<void> {
    await db.delete(qrcVisitaAmbienteImages)
      .where(and(eq(qrcVisitaAmbienteImages.tenantId, tenantId), eq(qrcVisitaAmbienteImages.id, id)));
  }

  private mapToEntity(row: typeof qrcVisitaAmbienteImages.$inferSelect): VisitaAmbienteImage {
    return {
      id:           row.id,
      tenantId:     row.tenantId,
