...
const TAG_CATALOG: Record<string, TagDefinition[]> = {
  ...
  EMAIL_CENTRAL_OFFLINE: [
    ...
    { tag: '{{gateway.name}}',           label: 'Nome da central',                 description: 'Nome da central que ficou offline',                          example: 'Central Moxuara 01' },
    ...
    { tag: '{{rule.emails}}',            label: 'Emails notificados',              description: 'Dentro de {{#each rules}} — lista separada por vírgula',     example: 'ops@cliente.com.br, suporte@myio.com.br' },
    { tag: '{{#each rule.devices}}',     label: 'Loop — abre bloco de centrais',   description: 'Dentro de {{#each rules}} — repete para cada central',       example: '' },
    { tag: '{{device.name}}',            label: 'Nome da central',                 description: 'Dentro de {{#each rule.devices}}',                           example: 'Central Moxuara 01' },
    { tag: '{{device.status}}',          label: 'Status da central',               description: "Dentro de {{#each rule.devices}} — 'OFFLINE'",               example: 'OFFLINE' },
    { tag: '{{device.timestamp}}',       label: 'Offline desde',                   description: 'Dentro de {{#each rule.devices}} — desde quando está offline', example: '16/09/2026 10:42' },
    { tag: '{{/each}}',                  label: 'Loop — fecha bloco',              description: 'Fecha {{#each rules}} ou {{#each rule.devices}}',            example: '' },
  ],
  EMAIL_REPORT: [
