...
const TAG_CATALOG: Record<string, TagDefinition[]> = {
  ...
  EMAIL_CENTRAL_OFFLINE: [
    { tag: '{{summary.centralsCount}}',  label: 'Total de centrais offline',       description: 'Mostrado no header — total de centrais afetadas',            example: '3' },
    { tag: '{{summary.rulesCount}}',     label: 'Qtd. de rules de central offline', description: 'Número total de rules de central offline disparadas',       example: '1' },
    { tag: '{{emails}}',                 label: 'Notificados no header (opcional)', description: 'Envolva em {{#if emails}}…{{/if}} — string separada por vírgula', example: 'ops@moxuara.com.br, suporte@myio.com.br' },
    { tag: '{{#each rules}}',            label: 'Loop — abre bloco de rules',      description: 'Repete o bloco HTML para cada rule de central offline',      example: '' },
    { tag: '{{rule.name}}',              label: 'Nome da rule',                    description: 'Dentro de {{#each rules}}',                                  example: 'Central offline > 150 min' },
    { tag: '{{rule.description}}',       label: 'Descrição da rule',               description: 'Dentro de {{#each rules}}',                                  example: 'Notifica quando a central fica sem comunicar' },
    { tag: '{{rule.condition}}',         label: 'Condição da rule',                description: 'Dentro de {{#each rules}}',                                  example: 'offline por 150 minutos' },
    { tag: '{{#each rule.centrals}}',    label: 'Loop — abre bloco de centrais',   description: 'Dentro de {{#each rules}} — repete para cada central offline', example: '' },
    { tag: '{{central.label}}',          label: 'Label da central (principal)',    description: 'Dentro de {{#each rule.centrals}} — nome amigável exibido',  example: 'Central Loja Q303A' },
    { tag: '{{central.name}}',           label: 'Nome técnico da central',         description: 'Dentro de {{#each rule.centrals}} — exibido entre parênteses', example: 'MOX_CENTRAL_01' },
    { tag: '{{central.status}}',         label: 'Status da central',               description: "Dentro de {{#each rule.centrals}} — 'OFFLINE'",              example: 'OFFLINE' },
    { tag: '{{central.timestamp}}',      label: 'Offline desde',                   description: 'Dentro de {{#each rule.centrals}} — desde quando está offline', example: '16/09/2026 10:42' },
    { tag: '{{central.mac}}',            label: 'MAC da central (opcional)',       description: 'Dentro de {{#each rule.centrals}} — envolva em {{#if central.mac}}', example: '00:1A:2B:3C:4D:5E' },
    { tag: '{{central.ipv6}}',           label: 'IPv6 da central (opcional)',      description: 'Dentro de {{#each rule.centrals}} — envolva em {{#if central.ipv6}}', example: 'fe80::1a2b:3c4d:5e6f:7a8b' },
    { tag: '{{central.uid}}',            label: 'UID da central (opcional)',       description: 'Dentro de {{#each rule.centrals}} — envolva em {{#if central.uid}}', example: 'gw-7f3a1c9e' },
    { tag: '{{observation}}',            label: 'Observação (opcional)',           description: 'Seção final — envolva em {{#if observation}}…{{/if}}',       example: 'Equipe de campo acionada.' },
    { tag: '{{#each workOrders}}',       label: 'Loop — Ordens de Serviço (opc.)', description: 'Seção final — envolva em {{#if workOrders}}…{{/if}}',        example: '' },
    { tag: '{{workOrder.number}}',       label: 'Número da OS',                    description: 'Dentro de {{#each workOrders}}',                            example: 'OS-2087' },
    { tag: '{{workOrder.title}}',        label: 'Título da OS',                    description: 'Dentro de {{#each workOrders}}',                            example: 'Verificar conectividade da central' },
    { tag: '{{workOrder.status}}',       label: 'Status da OS',                    description: 'Dentro de {{#each workOrders}}',                            example: 'Aberta' },
    { tag: '{{/each}}',                  label: 'Loop — fecha bloco',              description: 'Fecha {{#each rules}} / {{#each rule.devices}} / {{#each workOrders}}', example: '' },
  ],
  INCIDENT: [
    { tag: '{{summary.customersCount}}', label: 'Qtd. de clientes',                description: 'Total de clientes com incidente',                           example: '2' },
    { tag: '{{summary.gatewaysCount}}',  label: 'Qtd. de gateways',                description: 'Total de gateways afetados',                                example: '3' },
    { tag: '{{summary.devicesCount}}',   label: 'Qtd. de devices',                 description: 'Total de devices afetados',                                 example: '4' },
    { tag: '{{summary.slotsCount}}',     label: 'Qtd. de slots',                   description: 'Total de slots com gap',                                    example: '6' },
    { tag: '{{summary.detectedAt}}',     label: 'Detectado em',                    description: 'Data/hora da detecção pelo Agente do Ingestion',            example: '16/09/2026 11:20' },
    { tag: '{{summary.origin}}',         label: 'Origem do incidente',             description: 'Agente de interpolação ou execução manual (usuário)',       example: 'Agente de interpolação' },
    { tag: '{{emails}}',                 label: 'Emails notificados',              description: 'Destinatários — string separada por vírgula e espaço',      example: 'admin@myio.com.br, ops@moxuara.com.br' },
    { tag: '{{#each customers}}',        label: 'Loop — clientes',                 description: 'Repete o bloco para cada cliente',                          example: '' },
    { tag: '{{customer.name}}',          label: 'Nome do cliente',                 description: 'Dentro de {{#each customers}}',                             example: 'Moxuara Shopping' },
    { tag: '{{#each customer.gateways}}',label: 'Loop — gateways do cliente',      description: 'Dentro de {{#each customers}}',                             example: '' },
    { tag: '{{gateway.name}}',           label: 'Nome do gateway',                 description: 'Dentro de {{#each customer.gateways}}',                     example: 'Central Moxuara 01' },
    { tag: '{{gateway.mac}}',            label: 'MAC do gateway (opcional)',       description: 'Envolva em {{#if gateway.mac}}…{{/if}} para ocultar quando ausente', example: '00:1A:2B:3C:4D:5E' },
    { tag: '{{gateway.ipv6}}',           label: 'IPv6 do gateway (opcional)',      description: 'Envolva em {{#if gateway.ipv6}}…{{/if}}',                   example: 'fe80::1a2b:3c4d:5e6f:7a8b' },
    { tag: '{{gateway.uid}}',            label: 'UID do gateway (opcional)',       description: 'Envolva em {{#if gateway.uid}}…{{/if}}',                    example: 'gw-7f3a1c9e' },
    { tag: '{{#each gateway.devices}}',  label: 'Loop — devices do gateway',       description: 'Dentro de {{#each customer.gateways}}',                     example: '' },
    { tag: '{{device.label}}',           label: 'Label do device (principal)',     description: 'Dentro de {{#each gateway.devices}} — nome amigável exibido', example: 'Outback' },
    { tag: '{{device.name}}',            label: 'Nome técnico do device',          description: 'Dentro de {{#each gateway.devices}} — exibido entre parênteses', example: '3F SCSMO_L2_330' },
    { tag: '{{#each device.slots}}',     label: 'Loop — slots do device',          description: 'Dentro de {{#each gateway.devices}}',                       example: '' },
    { tag: '{{slot.slot}}',              label: 'Hora',                            description: 'Dentro de {{#each device.slots}} — a hora do slot',         example: '13:00' },
    { tag: '{{slot.value}}',             label: 'Valor interpolado',               description: 'Dentro de {{#each device.slots}} — valor preenchido',       example: '450 kWh' },
    { tag: '{{observation}}',            label: 'Observação (opcional)',           description: 'Seção final — envolva em {{#if observation}}…{{/if}}',       example: 'Interpolação aplicada por queda de energia na região.' },
    { tag: '{{#each workOrders}}',       label: 'Loop — Ordens de Serviço (opc.)', description: 'Seção final — envolva em {{#if workOrders}}…{{/if}}',        example: '' },
    { tag: '{{workOrder.number}}',       label: 'Número da OS',                    description: 'Dentro de {{#each workOrders}}',                            example: 'OS-1042' },
    { tag: '{{workOrder.title}}',        label: 'Título da OS',                    description: 'Dentro de {{#each workOrders}}',                            example: 'Verificar central Moxuara 01' },
    { tag: '{{workOrder.status}}',       label: 'Status da OS',                    description: 'Dentro de {{#each workOrders}}',                            example: 'Aberta' },
    { tag: '{{/each}}',                  label: 'Loop — fecha bloco',              description: 'Fecha qualquer {{#each}} (customers/gateways/devices/slots/workOrders)', example: '' },
  ],
  EMAIL_REPORT: [
  ...
  TELEGRAM_DAILY_SUMMARY_MULTI: [
    { tag: '{{report.date}}',                 label: 'Data do relatório',              description: 'Data do resumo diário',                                              example: '08/04/2026' },
    { tag: '{{report.generatedAt}}',          label: 'Hora de geração',                description: 'Hora em que o resumo foi gerado',                                    example: '08/04/2026 23:59:00' },
    { tag: '{{summary.totalCustomers}}',      label: 'Total de clientes',              description: 'Total de customers monitorados',                                     example: '8' },
    { tag: '{{summary.totalAlarmsActive}}',   label: 'Total de alarmes ativos',        description: 'Alarmes ativos somados de todos os customers',                       example: '5' },
    { tag: '{{summary.totalDevicesOffline}}', label: 'Total de devices offline',       description: 'Devices offline somados de todos os customers',                      example: '12' },
    { tag: '{{#each customers}}',             label: 'Loop — clientes',                description: 'Repete o bloco para cada customer',                                  example: '' },
    { tag: '{{customer.statusIcon}}',         label: 'Ícone de status',                description: 'Calculado pelo alarm backend: 🔴 alarmes ativos | 🟡 offline | 🟢 ok', example: '🔴' },
    { tag: '{{customer.name}}',               label: 'Nome do cliente',                description: 'Dentro de {{#each customers}}',                                      example: 'Moxuara' },
    { tag: '{{customer.alarmsActive}}',       label: 'Alarmes ativos',                 description: 'Dentro de {{#each customers}}',                                      example: '2' },
    { tag: '{{customer.alarmsOpened}}',       label: 'Alarmes abertos hoje',           description: 'Dentro de {{#each customers}}',                                      example: '3' },
    { tag: '{{customer.devicesOffline}}',     label: 'Devices offline',                description: 'Dentro de {{#each customers}}',                                      example: '4' },
    { tag: '{{/each}}',                       label: 'Fecha loop',                     description: 'Fecha {{#each customers}}',                                           example: '' },
  ],
  TELEGRAM_DAILY_SUMMARY: [
    { tag: '{{report.date}}',              label: 'Data do relatório',        description: 'Data do resumo diário',                                          example: '08/04/2026' },
    { tag: '{{report.generatedAt}}',       label: 'Hora de geração',          description: 'Hora em que o resumo foi gerado',                                example: '08/04/2026 23:59:00' },
    { tag: '{{customer.name}}',            label: 'Nome do cliente',          description: 'Nome do customer',                                               example: 'Myio' },
    { tag: '{{summary.alarmsOpened}}',     label: 'Alarmes abertos no dia',   description: 'Total de alarmes abertos durante o dia',                         example: '5' },
    { tag: '{{summary.alarmsClosed}}',     label: 'Alarmes resolvidos',       description: 'Total de alarmes fechados durante o dia',                        example: '3' },
    { tag: '{{summary.alarmsActive}}',     label: 'Alarmes ainda ativos',     description: 'Alarmes ainda abertos ao fim do dia',                            example: '2' },
    { tag: '{{summary.devicesOnline}}',    label: 'Dispositivos online',      description: 'Dispositivos online ao fim do dia',                              example: '142' },
    { tag: '{{summary.devicesOffline}}',   label: 'Dispositivos offline',     description: 'Dispositivos offline ao fim do dia',                             example: '3' },
    { tag: '{{#if summary.alarmsActive}}', label: 'Condicional — há ativos',  description: 'Renderiza bloco somente se há alarmes ainda abertos',            example: '' },
    { tag: '{{#each activeAlarms}}',       label: 'Loop — alarmes ativos',    description: 'Repete o bloco para cada alarme ainda aberto',                   example: '' },
    { tag: '{{alarm.title}}',              label: 'Título do alarme',         description: 'Dentro de {{#each activeAlarms}}',                               example: 'Sem Leitura 12h' },
    { tag: '{{alarm.raisedAt}}',           label: 'Data/hora de abertura',    description: 'Dentro de {{#each activeAlarms}}',                               example: '08/04/2026 10:22:00' },
    { tag: '{{/each}}',                    label: 'Fecha loop',               description: 'Fecha {{#each activeAlarms}}',                                   example: '' },
    { tag: '{{/if}}',                      label: 'Fecha condicional',        description: 'Fecha {{#if}}',                                                  example: '' },
  ],
  INSIGHT: [
