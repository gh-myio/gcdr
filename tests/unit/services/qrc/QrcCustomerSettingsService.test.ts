import { QrcCustomerSettingsService } from '../../../../src/services/qrc/QrcCustomerSettingsService';
import { IQrcCustomerSettingsRepository } from '../../../../src/repositories/interfaces/qrc/IQrcCustomerSettingsRepository';
import { ICustomerRepository } from '../../../../src/repositories/interfaces/ICustomerRepository';
import { QrcCustomerSettings } from '../../../../src/domain/entities/qrc/CustomerSettings';
import { Customer } from '../../../../src/domain/entities/Customer';
import { NotFoundError } from '../../../../src/shared/errors/AppError';

describe('QrcCustomerSettingsService', () => {
  const tenantId   = 't-1';
  const customerId = 'c-1';
  const userId     = 'user-1';

  let settingsRepo: jest.Mocked<IQrcCustomerSettingsRepository>;
  let customerRepo: jest.Mocked<ICustomerRepository>;
  let service: QrcCustomerSettingsService;

  const mockCustomer = { id: customerId, tenantId } as unknown as Customer;
  const mockSettings: QrcCustomerSettings = {
    customerId,
    tenantId,
  ...
  beforeEach(() => {
    ...
    service = new QrcCustomerSettingsService(settingsRepo, customerRepo);
  });
  ...
  describe('enable', () => {
    ...
    it('passes null viewerPasswordHash when no password is given', async () => {
      ...
      expect(settingsRepo.enable).toHaveBeenCalledWith(
        tenantId, customerId,
        expect.objectContaining({ viewerPasswordHash: null, defaultCentralId: null, qrcMetadata: {} }),
        userId,
      );
    });
