import {
  ...
  pinColumnsForWrite,
  constantTimeEqual,
} from '../../../../src/services/qrc/QrcPinService';
...
beforeAll(() => {
  process.env.QRC_PIN_PEPPER = 'test-pepper-32-chars-min-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
});

describe('QrcPinService', () => {
  ...
  describe('pinLookupToken', () => {
    ...
    it('throws if pepper is missing or too short', () => {
      const saved = process.env.QRC_PIN_PEPPER;
      try {
        process.env.QRC_PIN_PEPPER = 'short';
        expect(() => pinLookupToken(tenantA, '1234')).toThrow(/QRC_PIN_PEPPER/);

        delete process.env.QRC_PIN_PEPPER;
        expect(() => pinLookupToken(tenantA, '1234')).toThrow(/QRC_PIN_PEPPER/);
      } finally {
        process.env.QRC_PIN_PEPPER = saved;
      }
    });
  });
