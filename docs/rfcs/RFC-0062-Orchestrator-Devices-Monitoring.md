...
> - ⬜ **Pending.** config/compose + operator docs (Phase 1 operability) → cockpit (Phase 2) → telemetry fan-out (Phase 2) → os-monitor (Phase 3). Real `/incidents/candidates` integration is a separate smoke once `ALARMS_API_URL`/token are pointed.
